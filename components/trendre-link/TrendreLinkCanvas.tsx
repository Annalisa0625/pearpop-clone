...
function getInitial(name: string) { return name.trim().slice(0, 1).toUpperCase() || "T"; }
...
export default function TrendreLinkCanvas({ data, mode, locale = "ja", editingField = null, onEditingFieldChange, onDisplayNameChange, onBioChange, onEditProfile, onEditInquirySettings, onAddFirstLink, onEditItem, onReorderItems, onReorderInquiryTypes }: CanvasProps) {
  ...
  const copy = locale === "ja" ? { editName: "表示名を編集", addBio: "＋ 自己紹介を追加", firstLink: "＋ 最初のリンクを追加", inquiries: "お問い合わせ", reorder: "並び替え", reordering: "並び替え中", done: "完了" } : { editName: "Edit display name", addBio: "+ Add a bio", firstLink: "+ Add your first link", inquiries: "Work inquiries", reorder: "Reorder", reordering: "Reordering", done: "Done" };
  ...
  return <div style={backgroundStyle} className={`relative min-h-[100dvh] w-full overflow-x-hidden ${theme.shell} ${fontClass}`}>
    ...
    <div className={`relative z-[1] mx-auto flex min-h-[100dvh] w-full max-w-[520px] flex-col ${isEdit ? "pb-[calc(6.75rem+env(safe-area-inset-bottom))]" : "pb-[max(1.5rem,env(safe-area-inset-bottom))]"}`}>
      <div className="h-14 bg-current opacity-[0.03]" />
      <section className={`-mt-5 px-4 text-center ${isEdit && !editingField ? "opacity-[0.94] saturate-[0.94]" : ""}`}><div className="relative mx-auto w-fit"><div className={`flex h-20 w-20 items-center justify-center overflow-hidden rounded-full text-[27px] font-medium shadow-sm ring-1 ring-white/25 ${avatarUrl ? "" : theme.button}`}>{avatarUrl ? <img src={avatarUrl} alt={page.displayName} draggable={false} className="h-full w-full object-cover" /> : getInitial(page.displayName)}</div>{isEdit ? <button type="button" onClick={onEditProfile} className={`absolute -bottom-1 -right-1 flex h-11 w-11 items-center justify-center rounded-full ${theme.subtle}`} aria-label={copy.editName}><span className={`flex h-7 w-7 items-center justify-center rounded-full border shadow-sm ${theme.edit}`}><PencilIcon /></span></button> : null}</div>
        <div className="relative mx-auto mt-3 max-w-sm">{isEdit && editingField === "displayName" ? <input autoFocus value={page.displayName} maxLength={80} onChange={(e) => onDisplayNameChange?.(e.target.value)} onBlur={() => onEditingFieldChange?.(null)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === "Escape") onEditingFieldChange?.(null); }} className={`w-full rounded-xl border px-3 py-2 text-center text-[23px] font-medium outline-none ${theme.edit}`} /> : isEdit ? <button type="button" onClick={() => onEditingFieldChange?.("displayName")} className="inline-flex max-w-full items-center gap-1.5 text-[23px] font-medium"><span className="truncate">{page.displayName || "Creator"}</span><span className={theme.subtle}><PencilIcon /></span></button> : <h1 className="text-[23px] font-medium">{page.displayName}</h1>}</div>
      ...
      {(items.length > 0 || isEdit) ? <section className="px-4 pt-5">{isEdit && (items.length > 0 || enabledForms.length > 1) ? <div className="mb-2 flex h-9 items-center justify-end gap-2"><span className={`text-xs ${theme.subtle}`}>{reorderMode ? copy.reordering : ""}</span><button type="button" onClick={() => setReorderMode((value) => !value)} className={`flex min-h-11 items-center gap-1 rounded-lg px-2 text-xs font-medium ${theme.subtle}`}>{reorderMode ? copy.done : <><DragIcon />{copy.reorder}</>}</button></div> : null}
        {items.length > 0 ? canSortItems ? <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={clearSelection} onDragEnd={(event: DragEndEvent) => { clearSelection(); const { active, over } = event; if (!over || active.id === over.id) return; const from = items.findIndex((item) => item.id === active.id); const to = items.findIndex((item) => item.id === over.id); if (from >= 0 && to >= 0) onReorderItems?.(arrayMove(items, from, to)); }}><SortableContext items={items.map((item) => item.id ?? "")} strategy={rectSortingStrategy}><div className="flex flex-wrap gap-2">{items.map((item) => <SortableShell key={item.id} id={item.id!} width={itemWidth(normalizeCreatorLinkItemAppearance(item.metadata))}><CanvasItem item={item} mode="edit" /></SortableShell>)}</div></SortableContext></DndContext> : <div className={`flex flex-wrap gap-2 ${isEdit ? "opacity-[0.94] saturate-[0.94]" : ""}`}>{items.map((item, index) => <div key={item.id ?? `${item.itemType}-${index}`} className={itemWidth(normalizeCreatorLinkItemAppearance(item.metadata))}><CanvasItem item={item} mode={mode} onEdit={() => onEditItem?.(item)} /></div>)}</div> : isEdit ? <button type="button" onClick={onAddFirstLink} className={`w-full rounded-2xl border border-dashed px-4 py-3 text-sm font-medium ${theme.edit}`}>{copy.firstLink}</button> : null}
      </section> : null}
