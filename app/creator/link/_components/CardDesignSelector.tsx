...
const DEPTH_LABELS: Record<CreatorLinkItemDepth, { ja: string; en: string }> = { normal: { ja: "標準", en: "Normal" }, soft: { ja: "ソフト", en: "Soft" }, raised: { ja: "立体", en: "Raised" } };
...
export default function CardDesignSelector({ value, onChange, locale }: { value: CreatorLinkItemAppearance; onChange: (value: CreatorLinkItemAppearance) => void; locale: "ja" | "en" }) {
  ...
  return <div className="space-y-5 border-t border-slate-200/70 pt-5">
    ...
    <fieldset><legend className="mb-2 text-sm font-medium text-slate-700">{locale === "ja" ? "奥行き" : "Depth"}</legend><div className="grid grid-cols-3 gap-2">{CREATOR_LINK_ITEM_DEPTHS.map((depth) => <button key={depth} type="button" aria-pressed={(value.depth ?? "normal") === depth} onClick={() => onChange({ ...value, depth })} className={`h-10 rounded-xl border text-xs font-medium transition ${(value.depth ?? "normal") === depth ? selected : idle}`}>{DEPTH_LABELS[depth][locale]}</button>)}</div></fieldset>
