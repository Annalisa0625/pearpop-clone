...
export default function CreatorLinkOnboarding(props: Props) {
  ...
  if (step === 0) {
    ...
    scene = <StepShell step={step} title="SNSをつなぎましょう" description="複数追加できます。あとから追加・編集もできます。" onBack={back} footer={<PrimaryButton onClick={() => props.onStepChange(4)}>{props.savedSocials.length ? "続ける" : "スキップして続ける"}</PrimaryButton>}><div className="space-y-5"><div className="flex justify-center gap-2">{SOCIALS.map((platform) => <button key={platform} type="button" onClick={() => setActiveSocial(platform)} className={`onboarding-press relative flex h-12 w-12 items-center justify-center rounded-full ${activeSocial === platform ? "bg-white text-black" : "bg-white/10 text-white"}`}><SocialBrandIcon platform={platform} />{props.savedSocials.includes(platform) ? <Check className="absolute -right-1 -top-1 h-4 w-4 rounded-full bg-emerald-400 p-0.5 text-black" /> : null}</button>)}</div><div className="flex gap-2"><input value={props.socialInputs[activeSocial]} onChange={(event) => props.onSocialChange(activeSocial, event.target.value)} placeholder="@username / URL" className="h-14 min-w-0 flex-1 rounded-2xl border border-white/14 bg-white/[.08] px-4 text-white outline-none" /><button type="button" disabled={props.socialSaving || !props.socialInputs[activeSocial].trim()} onClick={() => void props.onSaveSocial(activeSocial)} className="onboarding-press min-w-[78px] rounded-2xl bg-white px-3 text-sm font-semibold text-black disabled:bg-white/15 disabled:text-white/30">{saved ? "更新" : "追加"}</button></div></div></StepShell>;
  } else if (step === 4) {
