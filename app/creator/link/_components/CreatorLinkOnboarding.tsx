...
import { creatorLinkOnboardingDictionary } from "@/lib/i18n/creatorLink";
...
function StepShell({ step, title, description, backLabel, galleryMode = false, onBack, children, footer }: { step: number; title: string; description: string; backLabel: string; galleryMode?: boolean; onBack: () => void; children: ReactNode; footer: ReactNode }) {
...
function Phone({ data, locale }: { data: TrendreLinkCanvasData; locale: AppLocale }) {
...
export default function CreatorLinkOnboarding(props: Props) {
  ...
  const copy = creatorLinkOnboardingDictionary[locale];
  ...
  let scene: ReactNode;
  if (step === 0) {
    scene = <StepShell step={step} title={copy.displayTitle} description={copy.displayBody} backLabel={copy.back} onBack={back} footer={<PrimaryButton disabled={!props.form.displayName.trim()} onClick={() => props.onStepChange(1)}>{copy.continue}</PrimaryButton>}><label className="block text-[13px] font-medium text-white/60">{copy.displayName}<input autoFocus value={props.form.displayName} maxLength={80} onChange={(event) => props.onChange({ displayName: event.target.value })} className="mt-2 h-16 w-full rounded-2xl border border-white/14 bg-white/[.08] px-5 text-lg font-semibold text-white outline-none focus:border-white/45" /></label></StepShell>;
  } else if (step === 1) {
    const available = props.slugState === "available" && !props.slugError;
    const slugMessage = props.slugError
      ? copy.checkFailed
      : props.slugState === "checking"
        ? copy.checking
        : props.slugState === "available"
          ? copy.available
          : props.slugState === "unavailable"
            ? copy.unavailable
            : copy.invalid;
    scene = <StepShell step={step} title={copy.urlTitle} description={copy.urlBody} backLabel={copy.back} onBack={back} footer={<PrimaryButton disabled={!available} onClick={() => props.onStepChange(2)}>{copy.continue}</PrimaryButton>}><div className="flex h-16 overflow-hidden rounded-2xl border border-white/14 bg-white/[.08] focus-within:border-white/45"><span className="flex items-center border-r border-white/10 px-3 text-[13px] text-white/45">trendre.jp/in/</span><input value={props.form.slug} maxLength={30} onChange={(event) => props.onChange({ slug: event.target.value })} className="min-w-0 flex-1 bg-transparent px-3 text-[17px] font-semibold text-white outline-none" /></div><p className={`mt-3 text-[13px] ${available ? "text-emerald-300" : "text-rose-300"}`}>{slugMessage}</p></StepShell>;
  } else if (step === 2) {
    scene = <StepShell step={step} title={copy.photoTitle} description={copy.photoBody} backLabel={copy.back} onBack={back} footer={<PrimaryButton onClick={() => props.onStepChange(3)}>{props.form.avatarUrl ? copy.continue : copy.addLater}</PrimaryButton>}><div className="flex flex-col items-center"><label className="onboarding-press relative flex h-44 w-44 cursor-pointer items-center justify-center overflow-hidden rounded-full bg-white/10 ring-1 ring-white/20 focus-within:ring-4 focus-within:ring-white/25">{props.form.avatarUrl ? <img src={props.form.avatarUrl} alt={copy.profilePhoto} className="h-full w-full object-cover" /> : <span className="flex flex-col items-center gap-2 text-white/60"><Plus className="h-8 w-8" />{copy.choosePhoto}</span>}<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={props.uploadingImage} onChange={(event) => { const file = event.target.files?.[0]; if (file) props.onSelectAvatar(file); event.currentTarget.value = ""; }} />{props.form.avatarUrl ? <span className="absolute bottom-2 right-2 flex h-11 w-11 items-center justify-center rounded-full bg-white text-black"><Pencil className="h-4 w-4" /></span> : null}</label><p className="mt-5 flex items-center gap-2 text-xs text-white/45"><Images className="h-4 w-4" />{copy.photoFormats}</p></div></StepShell>;
  } else if (step === 3) {
    const saved = props.savedSocials.includes(activeSocial);
    const service = getCreatorLinkService(activeSocial);
    scene = <StepShell step={step} title={copy.socialTitle} description={copy.socialBody} backLabel={copy.back} onBack={back} footer={<PrimaryButton onClick={() => props.onStepChange(4)}>{props.savedSocials.length ? copy.continue : copy.skipSocial}</PrimaryButton>}><div className="space-y-5"><div className="flex justify-center gap-2">{SOCIALS.map((platform) => <button key={platform} type="button" onClick={() => setActiveSocial(platform)} aria-label={getCreatorLinkService(platform).labelEn} className={`onboarding-press relative flex h-12 w-12 items-center justify-center rounded-full ${activeSocial === platform ? "bg-white text-black" : "bg-white/10 text-white"}`}><SocialBrandIcon platform={platform} />{props.savedSocials.includes(platform) ? <Check className="absolute -right-1 -top-1 h-4 w-4 rounded-full bg-emerald-400 p-0.5 text-black" /> : null}</button>)}</div><div className="flex gap-2"><div className="flex h-14 min-w-0 flex-1 overflow-hidden rounded-2xl border border-white/14 bg-white/[.08] focus-within:border-white/45"><span className="flex shrink-0 items-center border-r border-white/10 px-2.5 text-xs text-white/45">{service.displayPrefix}</span><input value={props.socialInputs[activeSocial]} onChange={(event) => { const raw = event.target.value; const normalized = normalizeCreatorLinkServiceInput(activeSocial, raw); props.onSocialChange(activeSocial, normalized.ok ? normalized.value.editableValue : raw.replace(/^@+/, "")); }} placeholder={service.placeholder} autoCapitalize="none" autoCorrect="off" spellCheck={false} className="min-w-0 flex-1 bg-transparent px-3 text-white outline-none" /></div><button type="button" disabled={props.socialSaving || !props.socialInputs[activeSocial].trim()} onClick={() => void props.onSaveSocial(activeSocial)} className="onboarding-press min-w-[78px] rounded-2xl bg-white px-3 text-sm font-semibold text-black disabled:bg-white/15 disabled:text-white/30">{saved ? copy.update : copy.add}</button></div></div></StepShell>;
  } else if (step === 4) {
    scene = <StepShell step={step} title={copy.firstLinkTitle} description={copy.firstLinkBody} backLabel={copy.back} onBack={back} footer={<div className="space-y-1"><PrimaryButton onClick={props.onAddLink}><Link2 className="h-4 w-4" />{copy.addLink}</PrimaryButton><button type="button" onClick={() => props.onStepChange(5)} className="min-h-11 w-full text-sm text-white/50">{copy.addLinkLater}</button></div>}><Phone data={props.previewData} locale={locale} /></StepShell>;
  } else if (step === 5) {
    scene = <StepShell step={step} galleryMode title={copy.styleTitle} description={copy.styleBody} backLabel={copy.back} onBack={back} footer={<PrimaryButton disabled={!selectedPresetId} onClick={() => props.onStepChange(6)}>{copy.useStyle}</PrimaryButton>}><StylePresetGallery data={props.previewData} selectedPresetId={selectedPresetId} onSelect={(preset) => { const previousPresetId = selectedPresetId; setSelectedPresetId(preset.id); void props.onApplyPreset(preset).then((saved) => { if (!saved) setSelectedPresetId(previousPresetId); }); }} /></StepShell>;
  } else {
    scene = <StepShell step={step} title={copy.completeTitle} description={copy.completeBody} backLabel={copy.back} onBack={back} footer={props.completionReady ? <div className="space-y-1"><PrimaryButton onClick={props.onFinish}>{copy.keepEditing}</PrimaryButton><Link href={`/in/${props.form.slug}`} target="_blank" className="flex min-h-11 items-center justify-center gap-2 text-sm font-semibold text-white/65"><ExternalLink className="h-4 w-4" />{copy.viewPublicPage}</Link></div> : <PrimaryButton disabled={props.completing} onClick={() => void props.onComplete()}>{props.completing ? copy.publishing : copy.publish}</PrimaryButton>}><div><Phone data={props.previewData} locale={locale} /><div className="mx-auto mt-4 flex max-w-[330px] items-center gap-2 rounded-2xl border border-white/12 bg-white/[.07] p-2 pl-4"><span className="min-w-0 flex-1 truncate text-xs text-white/60">{props.publicUrl}</span><button type="button" onClick={props.onCopyPublicUrl} className="flex h-10 items-center gap-1 rounded-xl bg-white/10 px-3 text-xs"><Copy className="h-3.5 w-3.5" />{copy.copy}</button></div></div></StepShell>;
  }
  return <>{styles}{scene}</>;
}
