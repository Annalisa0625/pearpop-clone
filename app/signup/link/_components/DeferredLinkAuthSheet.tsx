...
export default function DeferredLinkAuthSheet({ open, busy = false, error, onClose, onGoogle, onEmail }: Props) {
  ...
  if (!open) return null;
  const submit = (event: FormEvent) => { event.preventDefault(); onEmail(email, password); };
  ...
  return <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/50 pt-[max(1rem,env(safe-area-inset-top))] backdrop-blur-[2px] sm:p-4" role="dialog" aria-modal="true" aria-labelledby="link-auth-title">
    ...
    <section className="trendre-auth-sheet relative z-[1] flex max-h-[min(90dvh,720px)] w-full max-w-[456px] flex-col overflow-hidden rounded-t-[30px] bg-[#fbfaf8] text-[#242326] shadow-[0_-18px_70px_rgba(0,0,0,.28)] sm:rounded-[30px]">
