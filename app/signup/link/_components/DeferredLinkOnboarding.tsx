...
import { CREATOR_LINK_SOCIAL_SERVICES, extractCreatorLinkServiceEditableValue, getCreatorLinkService, normalizeCreatorLinkServiceInput } from "@/lib/trendre-link/service-registry";
...
const socialLabel = (platform: CreatorLinkSocialPlatform) => getCreatorLinkService(platform).labelEn;
const socialPrefix = (platform: CreatorLinkSocialPlatform) => getCreatorLinkService(platform).displayPrefix;
const createId = () => typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`;

function socialInputValue(platform: CreatorLinkSocialPlatform, value: string) {
  return extractCreatorLinkServiceEditableValue(platform, value);
}
...
export default function DeferredLinkOnboarding({ draft, avatarPreviewUrl, slugState, slugMessage, publishedSlug, onChange, onAvatar, onRequireAuth }: Props) {
  ...
  useEffect(() => {
    ...
    setSocialValue(saved ? socialInputValue(activeSocial, saved.url) : "");
    setSocialError(null);
  ...
  const saveSocial = () => {
    ...
    setSocialValue(socialInputValue(activeSocial, normalized.value.url));
    setSocialError(null);
  };
