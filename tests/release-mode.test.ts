...
test("C-only blocks B APIs, checkout APIs, and formal quote actions", () => {
  for (const path of ["/api/b/requests/create", "/api/company/quotes/a/checkout", "/api/requests/create", "/api/public/inquiries", "/api/orders/checkout", "/api/stripe/checkout", "/api/stripe/webhook", "/api/creator/orders/inquiries/a/quote"]) {
    assert.equal(isCreatorOnlyBlockedApiPath(path), true, path);
  }
  for (const path of ["/api/creator/link/page", "/api/public/creator-link/inquiries", "/api/notifications", "/api/auth/callback"]) {
