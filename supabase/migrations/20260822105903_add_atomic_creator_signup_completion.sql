-- Complete a Creator's Trend Mart signup atomically. This function is called
-- only by the server-side signup API with a user ID derived from Supabase Auth.
...
begin
  -- A stable transaction-scoped lock serializes completion attempts for one
  -- Auth user without blocking other Creators.
  ...
  -- Completed accounts are immutable from this endpoint, including legacy
  -- Creator + Company role combinations.
  ...
  delete from public.creator_portfolio_assets
  ...
  delete from public.creator_menus
  where creator_id = v_creator_id;
  ...
  -- These writes are all in the same transaction. The marker is intentionally
  -- the final application-data write, after every required artifact exists.
  update public.creators
  set
